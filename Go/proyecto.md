...
El backend del sistema se construye sobre **Go (>= 1.22)** como lenguaje de programacion, aprovechando su rendimiento nativo, concurrencia de primera clase con goroutines y compilacion a binario estatico para despliegues simples. **Gin** se utiliza como framework HTTP por su rendimiento superior (comparable a frameworks C) y su ecosistema maduro de middleware.

**PostgreSQL** funciona como la base de datos relacional principal, garantizando transacciones ACID para todas las operaciones criticas del sistema: ventas, movimientos de inventario, operaciones de caja y facturacion. Se accede mediante **pgx** como driver nativo de alto rendimiento y **GORM** como ORM para operaciones CRUD estandar, con la posibilidad de usar raw SQL via pgx para queries criticas de rendimiento. Las migraciones de esquema se gestionan con **golang-migrate**.

El sistema de autenticacion se implementa mediante JWT (JSON Web Tokens) siguiendo el estandar OAuth2, con roles diferenciados para cajeros, administradores y supervisores, utilizando la libreria **golang-jwt/jwt**. Cada endpoint esta protegido por permisos basados en rol via middleware de Gin.

Las tareas asincronas (facturacion AFIP, generacion PDF, envio email) se gestionan mediante un sistema de **worker pool con goroutines y channels**, respaldado por **Redis** como cola de mensajes persistente utilizando la libreria **go-redis**. Esto reemplaza la necesidad de un sistema externo como Celery, aprovechando la concurrencia nativa de Go.

La generacion de PDFs se realiza con **go-pdf/fpdf** o **jung-kurt/gofpdf**. El envio de emails se gestiona con **go-mail/mail** o **jordan-wright/email**.
...
La carga masiva CSV permite importar catalogos completos de proveedores. El archivo CSV debe seguir un formato estandar con columnas para codigo de barras, nombre del producto, precio de costo, precio de venta sugerido, unidades por bulto y categoria. El sistema valida cada fila mediante la libreria `encoding/csv` nativa de Go, reporta errores de formato o datos inconsistentes, y procesa las filas validas mediante upsert para evitar duplicados.
...
El sistema implementa manejo global de excepciones via middleware de Gin con recovery, que: nunca expone stack traces al cliente, registra todos los errores con contexto suficiente para debugging mediante **zerolog** o **zap** como logger estructurado, retorna mensajes de error descriptivos y consistentes, y distingue entre errores de validacion (422), errores de negocio (400), errores de autenticacion (401), errores de autorizacion (403) y errores internos (500).
...
El backend sigue una arquitectura en capas con separacion estricta de responsabilidades, organizado segun las convenciones de Go (Standard Go Project Layout). El area de `handler` agrupa los handlers HTTP organizados por dominio de negocio: ventas, productos, inventario, caja, facturacion, proveedores, usuarios y consulta de precios. El area de `service` contiene la logica de negocio pura: el servicio de ventas con logica de desarme automatico, el servicio de inventario con gestion jerarquica, el servicio de caja con ciclo de vida y arqueo, el servicio de facturacion con integracion AFIP, y el servicio de proveedores con actualizacion masiva. El area de `model` define las entidades GORM: Producto, ProductoHijo, Venta, VentaItem, SesionCaja, MovimientoCaja, Comprobante, Proveedor, Usuario. El area de `dto` define los structs de request/response para validacion de entrada y serializacion de salida con tags de validacion. El area de `repository` encapsula el acceso a datos con PostgreSQL. El area de `infra` encapsula las conexiones con Redis, AFIP y SMTP.
...
- **Backend Go**: Binario compilado que expone la API REST, ejecuta la logica de negocio, gestiona la base de datos y ejecuta workers internos para tareas asincronas. Al compilar a un binario estatico, el contenedor Docker es extremadamente liviano (< 20MB con multi-stage build desde `scratch` o `alpine`).
- **PostgreSQL**: La fuente de la verdad. Todas las transacciones ACID, inventario, ventas y datos fiscales persisten aqui.
- **Redis**: Cache de productos frecuentes y cola de mensajes para tareas asincronas (facturacion, email).
- **Nginx (Frontend Server)**: Sirve los archivos estaticos de la React App como PWA con manifest y service worker.

> **Nota**: A diferencia de la version Python, Go elimina la necesidad de un servicio Celery separado. Las tareas asincronas se ejecutan dentro del mismo binario del backend mediante goroutines y un worker pool, con Redis como backing store para persistencia de jobs. Esto reduce la cantidad de contenedores de 6 a 5.
...
Para desarrollo local, un archivo `docker-compose.yml` separado levanta todo el stack con hot-reload habilitado en backend (via **air**) y frontend.
...
La facturacion es asincrona por diseño: la venta se confirma instantaneamente y el comprobante se genera en segundo plano via goroutines del worker pool. Esto desacopla la experiencia del cajero de la disponibilidad de AFIP.
...
La latencia objetivo de 100ms para el registro de items se logra mediante indices en codigo de barras, cache de productos frecuentes en Redis, queries optimizadas con pool de conexiones pgx, y el rendimiento inherente de Go compilado (sin overhead de interpretacion ni GIL).

Go ofrece ventajas intrinsecas de rendimiento frente a Python para este caso de uso: tiempos de respuesta mas bajos por ser compilado, menor consumo de memoria, concurrencia nativa sin necesidad de frameworks externos (goroutines vs asyncio + Celery), y despliegue simplificado con un unico binario estatico.
